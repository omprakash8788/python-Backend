...
def run_server_in_process():
    ...
    p.start()
    time.sleep(2)
    print("Server is running on a separate process")
    yield
    p.terminate()
...
async def main(n:int=10):
    with run_server_in_process():
        ...
        print(f"Time taken to make {n} requests" f" to sync endpoint:{end - begin} seconds")
        begin = time.time()
        await make_requests_to_the_endpoint(n, "/async")
        end= time.time()
        print(f"Time taken to make {n} requests" f" to async endpoint:{end - begin} seconds")
